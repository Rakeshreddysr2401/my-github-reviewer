    ...
    for ai_response in state.llm_response.reviews:
        try:
            ...
            if line_number not in line_map:
                log.warning(f"Line {line_number} which comment generated by AI not found in the diff — ignoring")
                continue

            change = line_map[line_number]

            if not change.content.startswith("+"):
                log.warning(f"Line {line_number} is not an added line: {change.content} — ignoring")
                continue
            ...
            log.debug(f"Formatted comment: {comment}")
            comments.append(comment)

        except (KeyError, TypeError, ValueError) as e:
            log.error(f"Error creating comment from AI response: {e}, Response: {ai_response}")

    log.info(f"Formatted {len(comments)} valid comments")
    state.comments.extend(comments)

    # ✅ Advance to the next chunk
    state.current_chunk_index += 1

    # ❌ Do NOT set `state.done = True` here — it's handled in `get_next_chunk.py`
    return state
